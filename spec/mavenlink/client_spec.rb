...
describe Mavenlink::Client do
  describe "API V1" do
    ...
    it "uses oauth2 over headers by default" do
      @client = Mavenlink::Client.new(:access_token => "some_token", :api_version => 1)
      stub_request(:get, "https://mavenlink.local/api/v1/time_entries?per_page=5").
        with(:headers => {'Accept'=>'application/json', 'Authorization'=>'Bearer some_token', 'Accept-Encoding'=>'gzip;q=1.0,deflate;q=0.6,identity;q=0.3', 'User-Agent'=>'Ruby'}).
        to_return(:status => 200, :body => @time_entries_json, :headers => {})
      @client.time_entries(:per_page => 5)
      WebMock.should have_requested(:get, "https://mavenlink.local/api/v1/time_entries?per_page=5")
    end

    it "uses oauth2 over params when basic_auth is also being used" do
      @client = Mavenlink::Client.new(:access_token => "some_token", :api_version => 1, :basic_auth_username => "u", :basic_auth_password => "p")
      stub_request(:get, "https://u:p@mavenlink.local/api/v1/time_entries?per_page=5&bearer_token=some_token").
        to_return(:status => 200, :body => @time_entries_json, :headers => {})
      @client.time_entries(:per_page => 5)
      WebMock.should have_requested(:get, "https://u:p@mavenlink.local/api/v1/time_entries?per_page=5&bearer_token=some_token")
    end
  end

  describe "API V0" do
    before do
      @client = Mavenlink::Client.new(:user_id => "user_id", :api_token => "token")
    end
