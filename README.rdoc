...
The REST API and Ruby client are documented on this site.  We also have an early-release {PHP client}[https://github.com/mavenlink/mavenlink_php_api].
...
The Mavenlink Ruby client uses Faraday as an abstract networking library.  By default we use Net::HTTP, but you can select a different adaptor with the `:faraday_adaptor` option.  You may also set the `:timeout` and `:open_timeout` when creating the Client.

  > client = Mavenlink::Client.new(:user_id => <user_id>, :api_token => '<api_token>', :faraday_adaptor => :em_http, :timeout => 25)
